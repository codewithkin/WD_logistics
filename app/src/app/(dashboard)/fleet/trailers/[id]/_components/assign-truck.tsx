...
import { Check, ChevronsUpDown, Truck as TruckIcon, X } from "lucide-react";
...
import { assignTruckToTrailer, getAvailableTrucks } from "../../actions";
...
export function AssignTruck({ trailerId, currentTruckId, currentTruckName }: AssignTruckProps) {
    ...
    const handleSelect = async (truckId: string | null) => {
        setIsLoading(true);
        try {
            const result = await assignTruckToTrailer(trailerId, truckId);
            if (result.success) {
    ...
    const displayName = selectedTruck
        ? `${selectedTruck.registrationNo} - ${selectedTruck.make} ${selectedTruck.model}`
        : currentTruckName;
                ...
                <PopoverContent className="w-[var(--radix-popover-trigger-width)] p-0" align="start">
                    <Command>
                        ...
                        <CommandList className="max-h-60 overflow-y-auto">
                            ...
                            <CommandGroup>
                                {trucks.map((truck) => {
                                    const isAssignedElsewhere = truck.assignedTrailerId && truck.assignedTrailerId !== trailerId;
                                    return (
                                        <CommandItem
                                            key={truck.id}
